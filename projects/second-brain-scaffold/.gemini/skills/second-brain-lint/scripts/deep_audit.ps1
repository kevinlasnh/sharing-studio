...
function Get-ExpectedPermalink {
    ...
    if ($VaultPath -eq "index.md") {
        return "second-brain/index"
    }
    if ($VaultPath -eq "CLAUDE.md") {
        return "second-brain/claude"
    }
    if ($VaultPath -eq "AGENTS.md") {
        return "second-brain/agents"
    }
    if ($VaultPath -eq "GEMINI.md") {
        return "second-brain/gemini"
    }
    if ($VaultPath -match '^daily/(\d{4}-\d{2}-\d{2})\.md$') {
        return "second-brain/daily/$($Matches[1])"
    }
...
# Mechanical scans include wiki, daily, root router files, and raw Markdown violations.
...
$permalinkIssues = New-Object System.Collections.Generic.List[object]
...
foreach ($file in $wikiMarkdownFiles) {
    ...
    $expectedPermalink = Get-ExpectedPermalink -VaultPath $vaultPath
    if (-not [string]::IsNullOrWhiteSpace($expectedPermalink)) {
        if ($keys -notcontains "permalink") {
            $permalinkIssues.Add([pscustomobject]@{ file = $vaultPath; issue = "missing permalink"; expected = $expectedPermalink; actual = $null })
        } else {
            $actualPermalink = $frontmatter.values["permalink"].Trim().Trim("'`"")
            if ($actualPermalink -ne $expectedPermalink) {
                $permalinkIssues.Add([pscustomobject]@{ file = $vaultPath; issue = "permalink mismatch"; expected = $expectedPermalink; actual = $actualPermalink })
            }
        }
    }

    if ($kind -eq "domain-index") {
        foreach ($required in @("title", "type", "permalink", "domain", "created", "updated")) {
...
foreach ($specialPath in @("index.md", "CLAUDE.md", "AGENTS.md", "GEMINI.md")) {
    $specialFullPath = Join-Path -Path $resolvedRoot -ChildPath $specialPath
    if (-not (Test-Path -LiteralPath $specialFullPath -PathType Leaf)) {
        continue
    }
    $frontmatter = Get-FrontmatterInfo $specialFullPath
    $expectedPermalink = Get-ExpectedPermalink -VaultPath $specialPath
    if (-not $frontmatter.has_frontmatter -or $frontmatter.close_line -eq 0) {
        $frontmatterIssues.Add([pscustomobject]@{ file = $specialPath; issue = "missing-or-unclosed-frontmatter" })
        continue
    }
    $keys = @($frontmatter.keys)
    if ($keys -notcontains "permalink") {
        $permalinkIssues.Add([pscustomobject]@{ file = $specialPath; issue = "missing permalink"; expected = $expectedPermalink; actual = $null })
    } else {
        $actualPermalink = $frontmatter.values["permalink"].Trim().Trim("'`"")
        if ($actualPermalink -ne $expectedPermalink) {
            $permalinkIssues.Add([pscustomobject]@{ file = $specialPath; issue = "permalink mismatch"; expected = $expectedPermalink; actual = $actualPermalink })
        }
    }
}
...
$hiddenDirectoryIssues = New-Object System.Collections.Generic.List[object]
$allowedHiddenDirectories = @(".git", ".claude", ".agents", ".gemini", ".claudian", ".obsidian", ".trash", ".workflows", ".brv")
foreach ($directory in Get-ChildItem -LiteralPath $resolvedRoot -Directory -Force -ErrorAction SilentlyContinue) {
    if (-not $directory.Name.StartsWith(".", [System.StringComparison]::Ordinal)) {
        continue
    }
    $itemCount = @(Get-ChildItem -LiteralPath $directory.FullName -Force -Recurse -ErrorAction SilentlyContinue).Count
    if ($allowedHiddenDirectories -notcontains $directory.Name) {
        $hiddenDirectoryIssues.Add([pscustomobject]@{
            directory = $directory.Name
            issue = "top-level hidden directory is outside the vault scaffold allowlist"
            item_count = $itemCount
        })
        continue
    }
    if ($directory.Name -eq ".trash" -and $itemCount -gt 0) {
        $hiddenDirectoryIssues.Add([pscustomobject]@{
            directory = $directory.Name
            issue = ".trash is allowed only when empty"
            item_count = $itemCount
        })
    }
}
...
$expectedGraphGroups = @(
    ...
    [pscustomobject]@{ query = 'path:"templates/"'; rgb = 9141448 },
...
$hookDryRunIssues = New-Object System.Collections.Generic.List[object]
...
function Invoke-HookDryRun {
    ...
    $scriptPath = Join-Path -Path $resolvedRoot -ChildPath $ScriptRelativePath
    if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $ScriptRelativePath; issue = "hook script missing" })
        return
    }
    ...
    $env:CLAUDE_PROJECT_DIR = $resolvedRoot
    try {
        $json = $payload | ConvertTo-Json -Depth 10
        $output = $json | powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath 2>&1
        $text = ($output | Out-String).Trim()
    } finally {
        $env:CLAUDE_PROJECT_DIR = $oldProjectDir
    }

    $blocked = ($text -match '"permissionDecision"\s*:\s*"deny"' -or $text -match '"decision"\s*:\s*"block"')
    if ($Expected -eq "block" -and -not $blocked) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $ScriptRelativePath; path = $PathText; issue = "expected block but allowed"; output = $text })
    } elseif ($Expected -eq "allow" -and $blocked) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $ScriptRelativePath; path = $PathText; issue = "expected allow but blocked"; output = $text })
    }
}

function Invoke-ShellHookDryRun {
    param(
        [string]$CommandText,
        [string]$Expected,
        [string]$Name
    ...
    $scriptRelativePath = ".claude/scripts/shell-write-policy.ps1"
    $scriptPath = Join-Path -Path $resolvedRoot -ChildPath $scriptRelativePath
    if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $scriptRelativePath; issue = "hook script missing" })
        return
    ...
    $blocked = ($text -match '"permissionDecision"\s*:\s*"deny"')
    if ($Expected -eq "block" -and -not $blocked) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $scriptRelativePath; issue = "expected block but allowed"; output = $text })
    } elseif ($Expected -eq "allow" -and $blocked) {
        $hookDryRunIssues.Add([pscustomobject]@{ name = $Name; script = $scriptRelativePath; issue = "expected allow but blocked"; output = $text })
    }
}
...
title: Test Page
type: concept
permalink: second-brain/wiki/test-domain/test-page
---
...
$validDomainIndexContent = @'
---
title: Test Domain Index
type: index
permalink: second-brain/wiki/test-domain/index
---
...
Invoke-HookDryRun ".claude/scripts/wiki-path-policy.ps1" "wiki/log.md" $null "block" "deprecated scaffold blocked"
Invoke-HookDryRun ".claude/scripts/wiki-path-policy.ps1" "wiki/../wiki/log.md" $null "block" "traversal deprecated scaffold blocked"
Invoke-HookDryRun ".claude/scripts/wiki-prewrite-syntax-check.ps1" "wiki/Test/BadName.md" $validWikiContent "block" "bad wiki path schema blocked"
Invoke-HookDryRun ".claude/scripts/wiki-path-policy.ps1" "wiki/../AGENTS.md" $null "block" "traversal out of wiki blocked by path policy"
Invoke-HookDryRun ".claude/scripts/wiki-prewrite-syntax-check.ps1" "wiki/test-domain/test-page.md" "# Missing frontmatter" "block" "wiki full-file no frontmatter blocked"
Invoke-HookDryRun ".claude/scripts/wiki-prewrite-syntax-check.ps1" "wiki/test-domain/test-page.md" $validWikiContent "allow" "valid wiki content allowed"
Invoke-HookDryRun ".claude/scripts/wiki-prewrite-syntax-check.ps1" "wiki/test-domain/_index.md" $validDomainIndexContent "allow" "valid domain index allowed"
Invoke-HookDryRun ".claude/scripts/daily-no-link-policy.ps1" "daily/2099-01-01.md" "[[bad-link]]" "block" "daily wikilink blocked"
Invoke-HookDryRun ".claude/scripts/daily-no-link-policy.ps1" "daily/../daily/2099-01-01.md" "[[bad-link]]" "block" "daily traversal wikilink blocked"
Invoke-HookDryRun ".claude/scripts/daily-no-link-policy.ps1" "daily/not-a-date.md" "plain text only" "block" "daily non-date path blocked"
Invoke-HookDryRun ".claude/scripts/raw-link-policy.ps1" "wiki/test-domain/test-page.md" "See also [[raw/source.pdf]]." "block" "raw weak context blocked"
Invoke-HookDryRun ".claude/scripts/raw-link-policy.ps1" "wiki/test-domain/test-page.md" "Source file: [[raw/source.pdf]] supports this note." "allow" "valid raw provenance allowed"
Invoke-HookDryRun ".claude/scripts/raw-link-policy.ps1" "wiki/test-domain/_index.md" "- [[raw/source.pdf]]" "block" "domain index raw blocked"
Invoke-HookDryRun ".claude/scripts/raw-link-policy.ps1" "foo/../raw/x.md" "plain text" "block" "raw markdown traversal path blocked"
Invoke-ShellHookDryRun "Set-Content wiki/test-domain/test-page.md 'bad'" "block" "shell wiki markdown write blocked"
Invoke-ShellHookDryRun "cp source.png raw/source.png" "allow" "shell raw image copy allowed"

$issueCounts = [ordered]@{
    forbidden_scaffold = (Get-ItemCount $forbiddenScaffold)
    hidden_directory_issues = (Get-ItemCount $hiddenDirectoryIssues)
    filename_issues = (Get-ItemCount $filenameIssues)
    frontmatter_issues = (Get-ItemCount $frontmatterIssues)
    permalink_issues = (Get-ItemCount $permalinkIssues)
    ...
    hook_dry_run_issues = (Get-ItemCount $hookDryRunIssues)
}
...
[pscustomobject]@{
    ...
    hidden_directory_issues = $hiddenDirectoryIssues
    filename_issues = $filenameIssues
    frontmatter_issues = $frontmatterIssues
    permalink_issues = $permalinkIssues
    ...
    hook_dry_run_issues = $hookDryRunIssues
