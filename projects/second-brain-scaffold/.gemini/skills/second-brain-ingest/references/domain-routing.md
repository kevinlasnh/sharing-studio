...
2. Page-level retrieval:
   ...
   - Filter search results by `file_path` before deciding. Only `wiki/{domain}/{slug}.md` content pages are page candidates. `_index.md` and root `index.md` are navigation evidence only. Exclude `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.claude/**`, `.agents/**`, `.gemini/**`, `.claudian/**`, `.workflows/**`, `.brv/**`, `.obsidian/**`, `templates/**`, `daily/**`, `raw/**`, and deprecated scaffold paths as page candidates.
...
The `semantic coverage plan` comes from `semantic-fidelity.md` and is part of the same confirmation gate as domain placement. Do not present it as an optional appendix. If it contains P0 defer/discard, unresolved high-risk merge, ambiguous meta-correction, or source support gaps, surface those items as confirmation-required before any wiki write.
