...
description: External maintainer health-check and repair workflow for <your-username>'s Second Brain vault. Use when the user asks for repository-level health checks, "仓库健康度外部检查", full vault audit, Claude/Codex/Gemini router maintenance, local skill validation, hook/MCP/Obsidian scaffold checks, or an end-to-end clean/residual-risk verdict.
---
...
Use Simplified Chinese when replying to <your-username>.
...
- Write workflows must not leave Basic Memory dirty: ingest with journal closes through `second-brain-journal`, whose final checkpoint runs CLI `basic-memory reindex --project second-brain` for search + embeddings; ingest with deferred journal, lint fix pass, router/skill maintenance, and Markdown file moves must run status/reindex/status before claiming clean.
...
- The allowed top-level hidden directories are `.git`, `.claude`, `.obsidian`, and empty Obsidian `.trash`.
...
```powershell
python <quick-validate-script> .\.claude\skills\second-brain-ingest
python <quick-validate-script> .\.claude\skills\second-brain-query
python <quick-validate-script> .\.claude\skills\second-brain-lint
python <quick-validate-script> .\.claude\skills\second-brain-journal
python <quick-validate-script> .\.claude\skills\second-brain-graph-manager
python <quick-validate-script> .\.claude\skills\second-brain-vault-audit
```
...
- Hidden directory scan: only `.git`, `.claude`, `.obsidian`, and empty `.trash` are allowed.
...
- Basic Memory-first rule scan:
  ...
  - ingest/query filters exclude `CLAUDE.md`, `AGENTS.md`, and `GEMINI.md` as content candidates or answer sources
  ...
  - journal says it is the final sync point for ingest-with-journal and runs CLI `basic-memory reindex --project second-brain` after every journal write
...
- Basic Memory contract consistency:
  ...
  - Embedding `reindex_recommended` is not treated as routine ingest/query blocker when status and search are clean, but journal checkpoint still runs default CLI reindex to refresh embeddings after batch work.
...
- Missing Basic Memory-first freshness gate, one-shot reindex before fallback, file_path filtering, or closure wording in router/local skills.
...
- Hidden directories are only `.git`, `.claude`, `.obsidian`, and empty `.trash`.
- No forbidden scaffold paths exist.
- Hook dry-runs prove both blocking and allowed examples.
- Router and local skills consistently enforce Basic Memory-first semantic retrieval and Basic Memory clean-closure invariants.
